...
for restart in range(flags.n_restarts):
    ...
    def make_environment(images, labels, e):
        ...
        # Select all the 4s and all the 9s.
        in_hold_out = torch.logical_or(original_labels == 4, original_labels == 9)
        out_hold_out = torch.logical_and(original_labels != 4, original_labels != 9)
        return {
            'images': (images.float() / 255.)[out_hold_out].cuda(),
            'labels': labels[:, None][out_hold_out].cuda(),
            'shape_labels': shape_labels[out_hold_out].cuda(),
            'colours': colors[out_hold_out].cuda()
        }, {'images': (images.float() / 255.)[in_hold_out].cuda(),
            'labels': labels[:, None][in_hold_out].cuda(),
            'shape_labels': shape_labels[in_hold_out].cuda(),
            'colours': colors[in_hold_out].cuda()
            }

    envs = []
    in_env_0, out_env_0 = make_environment(mnist_train[0][::2], mnist_train[1][::2], 0.2)
    envs.append(in_env_0)
    in_env_1, out_env_1 = make_environment(mnist_train[0][1::2], mnist_train[1][1::2], 0.1)
    envs.append(in_env_1)
    #envs = [
    #    make_environment(mnist_train[0][::2], mnist_train[1][::2], 0.2)[0],
    #    make_environment(mnist_train[0][1::2], mnist_train[1][1::2], 0.1)[0]
    #]
    in_test_env, out_test_env = make_environment(mnist_val[0], mnist_val[1], 0.9)
    envs.append(in_test_env)
    envs.append(out_env_0)
    envs.append(out_env_1)
    envs.append(out_test_env)
    ...
    for step in range(flags.steps):
        ...
        for env_0_batch, env_1_batch in zip(env_0_loader, env_1_loader):
            ...
            test_logits, test_reps = mlp(envs[2]['images'])
            ...
            # Zero shot performance on the hold out sets.
            hold_out_env_0_logits, hold_out_env_0_reps = mlp(envs[3]['images'])
            envs[3]['nll'] = mean_nll(hold_out_env_0_logits, envs[3]['labels'])
            envs[3]['acc'] = mean_accuracy(hold_out_env_0_logits, envs[3]['labels'])
            envs[3]['penalty'] = batch_penalty(hold_out_env_0_logits, envs[3]['labels'])

            hold_out_env_1_logits, hold_out_env_1_reps = mlp(envs[4]['images'])
            envs[4]['nll'] = mean_nll(hold_out_env_1_logits, envs[4]['labels'])
            envs[4]['acc'] = mean_accuracy(hold_out_env_1_logits, envs[4]['labels'])
            envs[4]['penalty'] = batch_penalty(hold_out_env_1_logits, envs[4]['labels'])

            hold_out_test_logits, hold_out_test_reps = mlp(envs[5]['images'])
            envs[5]['nll'] = mean_nll(hold_out_test_logits, envs[5]['labels'])
            envs[5]['acc'] = mean_accuracy(hold_out_test_logits, envs[5]['labels'])
            envs[5]['penalty'] = batch_penalty(hold_out_test_logits, envs[5]['labels'])
            ...
            if step % 100 == 0:
                ...
                print("Hold out env 0 acc was: {}".format(envs[3]['acc']))
                print("Hold out env 1 acc was: {}".format(envs[4]['acc']))
                print("Hold out test acc was: {}".format(envs[5]['acc']))
